...
// Definitions
		...
		inline const Transform3 Transform3::rotationZYX(const Vector3& radiansXYZ)
		{
			...
			return Transform3(
				Vector3((cZ * cY), (sZ * cY), -sY),
				Vector3(((tmp0 * sX) - (sZ * cX)),
				((tmp1 * sX) + (cZ * cX)), (cY * sX)),
				Vector3(((tmp0 * cX) + (sZ * sX)),
				((tmp1 * cX) - (cZ * sX)), (cY * cX)),
				Vector3(0.0f)
			);
		}
		...
		{
			float trace, radicand, scale;
			float xx, yx, zx, xy, yy, zy, xz, yz, zz;
			float tmpx, tmpy, tmpz, tmpw, qx, qy, qz, qw;
		...
		{
			return Matrix3(cross(vec, mat.getCol0()),
